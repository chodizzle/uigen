...
vi.mock("../ToolInvocationBadge", () => ({
  ToolInvocationBadge: ({ toolInvocation }: { toolInvocation: { toolName: string; state: string; args?: Record<string, unknown> } }) => {
    const command = toolInvocation.args?.command as string | undefined;
    const path = toolInvocation.args?.path as string | undefined;
    const filename = path ? path.split("/").pop() : undefined;
    if (toolInvocation.toolName === "str_replace_editor" && command === "create") {
      return <div>{toolInvocation.state === "result" ? `Created ${filename}` : `Creating ${filename}`}</div>;
    }
    return <div>{toolInvocation.state === "result" ? `Ran ${toolInvocation.toolName}` : `Running ${toolInvocation.toolName}`}</div>;
  },
}));
